...
#include "democar_odom/democar_odom.hpp"
#include "democar_odom/odometry.hpp"

democarodom::democarodom() : Node("ackermann_odometry")
{
  // Setup Parameters
  ...
  pub_timer_ = this->create_wall_timer(interval, std::bind(&democarodom::timerCallback, this));
  ...
  auto wheel_radius = declare_parameter("wheel_radius", 0.125);
  ...
  joint_state_sub_ = this->create_subscription<JointState>("joint_states", 10,
                                                           std::bind(&democarodom::jointstateCallback, this, std::placeholders::_1));

  cmd_vel_sub_ = this->create_subscription<Twist>("cmd_vel", 10,
                                                  std::bind(&democarodom::cmdvelSubCallback, this, std::placeholders::_1));
  ...
  if (has_imu_heading_ == true)
    imu_sub_ = this->create_subscription<Imu>(
        "imu/data", 10,
        std::bind(&democarodom::imuSubCallback, this, std::placeholders::_1));
  else
    steering_angle_sub_ = this->create_subscription<Float64>(
        "steering_angle_middle", 10,
        std::bind(&democarodom::steeringAngleSubCallback, this, std::placeholders::_1));
...
void democarodom::jointstateCallback(const JointState::SharedPtr msg)
{
  for (auto i = 0; i < 6; i++)
  {
    if (msg->position[i] == 0)
...
void democarodom::steeringAngleSubCallback(const Float64::SharedPtr msg)
{
...
void democarodom::cmdvelSubCallback(const Twist::SharedPtr msg)
{
  linear_x = msg->linear.x;
  angular_z = msg->angular.z;
}

void democarodom::imuSubCallback(const Imu::SharedPtr msg)
{
  tf2::Quaternion q_;
...
void democarodom::odomUpdate(const rclcpp::Time &time)
{
...
void democarodom::publishOdomTopic(const rclcpp::Time &time)
{
...
void democarodom::timerCallback()
{
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<democarodom>());
  rclcpp::shutdown();
