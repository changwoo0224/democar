...
class DemoCarController : public rclcpp::Node {
private:
  bool verbose = false;
...
public:
  DemoCarController() : Node("democar_controller") {
    ...
    cmd_vel_sub = create_subscription<Twist>(
      "cmd_vel", 10,
      std::bind(&DemoCarController::cmd_vel_callback, this, std::placeholders::_1)
    );
...
int main(int argc, char **argv) {
  ...
  auto democar_controller = std::make_shared<DemoCarController>();

  rclcpp::spin(democar_controller);
  rclcpp::shutdown();
