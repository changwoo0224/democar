...
def generate_launch_description():
    ...
    gazebo_model_path = os.path.join(get_package_share_directory('democar_gazebo'), 'models')
    ...
    pkg_path = os.path.join(get_package_share_directory('democar_gazebo'))
    world_path = os.path.join(pkg_path, 'worlds', 'jackal_race.world')
    ...
    robot_description_content = Command(
        [
            ...
            PathJoinSubstitution(
                [FindPackageShare("democar_gazebo"), "urdf", "democar_body.urdf.xacro"]
            ),
        ]
    )
    ...
    democar_odom = Node(
        package='democar_odom',
        executable='democar_odom',
        name='democar_odom',
        output='log',
        parameters=[{
            "verbose" : False,
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[democar_controller],
            )
        ),
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=load_velocity_controller,
                on_exit=[democar_odom],
            )
        ),
        RegisterEventHandler(
            event_handler=OnProcessExit(
